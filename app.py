...
if st.button("تأكيد الحجز"):
    if not name or not phone:
        ...
        save_receipt(uploaded_file, selected_seat, name, phone)  # حفظ صورة الإيصال
            ...
            if remaining_now <= 0:
                ...
                if st.button("تسجيل الدفعة"):
                    if new_payment <= 0:
                        ...
                        save_receipt(new_receipt, complete_seat, complete_name, complete_phone)  # حفظ صورة إيصال الدفعة
    ...
    if admin_pass_input == ADMIN_PASSWORD:
        ...
        if len(receipt_files) == 0:
            ...
            with st.expander("عرض الإيصالات واحداً واحداً"):
                for fname in receipt_files:
                    # الاسم بيبقى: مقعدX_رقمالموبايل_الاسم_الوقت.jpg
                    parts = fname.replace(".jpg", "").split("_")
                    readable = " | ".join(parts[:-1]) if len(parts) > 1 else fname
                    st.image(os.path.join(RECEIPTS_DIR, fname), caption=readable, width=300)
