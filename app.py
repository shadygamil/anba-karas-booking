...
def extract_amount_from_receipt(image_file, target_phone):
    ...
    try:
        ...
        currency_positions = [m.start() for m in re.finditer(r"(?:جنيه|EGP|L\.?E\.?|LE)", text_without_phone, re.IGNORECASE)]
        for pos in currency_positions:
            window = text_without_phone[max(0, pos - 25):pos]
        ...
        # لو مفيش كلمة عملة قريبة من أي رقم، ندور على أي أرقام معقولة (نستبعد السنوات زي 2020-2099 والمراجع الطويلة)
        all_numbers = number_pattern.findall(text_without_phone)
        candidates = []
        for n in all_numbers:
            ...
            if val <= 0 or val > 99999:
                continue
            if 2000 <= val <= 2099 and val == int(val):  # على الأغلب سنة مش مبلغ
                continue
...
with st.expander("🔐 لوحة الإدارة (للمسؤول فقط)"):
    ...
    if admin_pass_input == ADMIN_PASSWORD:
        ...
        # نجيب آخر دفعة مسجلة لكل حجز (المبلغ المكتوب والمكتشف وحالة التطابق وملف الإيصال)
        last_entered, last_detected, last_status, last_file = [], [], [], []
        for _, row in df_all.iterrows():
            matching_logs = df_log[(df_log["رقم الكرسي"] == row["رقم الكرسي"]) & (df_log["رقم الموبايل"] == row["رقم الموبايل"])]
            if len(matching_logs) > 0:
                last_row = matching_logs.iloc[-1]
                last_entered.append(last_row["المبلغ المدخل يدوياً"])
                last_detected.append(last_row["المبلغ المكتشف من الإيصال"])
                last_status.append(last_row["حالة التطابق"])
                last_file.append(last_row["ملف الإيصال"])
            else:
                last_entered.append(None)
                last_detected.append(None)
                last_status.append("لا يوجد سجل دفعة")
                last_file.append("")

        df_all["آخر دفعة (مُدخل)"] = last_entered
        df_all["آخر دفعة (من الإيصال)"] = last_detected
        df_all["حالة تطابق آخر دفعة"] = last_status
        df_all["_ملف_إيصال"] = last_file  # عمود مخفي للاستخدام الداخلي بس

        st.subheader(f"📋 كل الحجوزات مع تفاصيل الدفع ({len(df_all)} حجز)")
        display_columns = [
            "رقم الكرسي", "الاسم رباعي", "رقم الموبايل", "المبلغ المدفوع", "المتبقي",
            "آخر دفعة (مُدخل)", "آخر دفعة (من الإيصال)", "حالة تطابق آخر دفعة"
        ]
        ...
        if len(df_all) > 0:
            row_labels = [
                f"مقعد {row['رقم الكرسي']} - {row['الاسم رباعي']} - {row['حالة تطابق آخر دفعة']}"
                for _, row in df_all.iterrows()
            ]
            chosen_label = st.selectbox("اختر الحجز اللي عايز تشوف صورة إيصاله", options=row_labels, key="admin_choose_receipt")
            chosen_idx = row_labels.index(chosen_label)
            chosen_filename = df_all.iloc[chosen_idx]["_ملف_إيصال"]
