...
def save_receipt(image_file, seat_num):
    """يحفظ صورة الإيصال في مجلد receipts باسم فيه رقم الكرسي والوقت"""
    try:
        image_file.seek(0)
        image = Image.open(image_file).convert("RGB")
        filename = f"seat_{seat_num}_{int(time.time())}.jpg"
...
if st.button("تأكيد الحجز"):
    if not name or not phone:
        ...
        save_receipt(uploaded_file, selected_seat)  # حفظ صورة الإيصال
            ...
            if remaining_now <= 0:
                ...
                if st.button("تسجيل الدفعة"):
                    if new_payment <= 0:
                        ...
                        save_receipt(new_receipt, complete_seat)  # حفظ صورة إيصال الدفعة
...
with st.expander("🔐 لوحة الإدارة (للمسؤول فقط)"):
    ...
    if admin_pass_input == ADMIN_PASSWORD:
        ...
        st.markdown("---")
        st.subheader("🧾 صور الإيصالات")
        receipt_files = sorted(os.listdir(RECEIPTS_DIR)) if os.path.exists(RECEIPTS_DIR) else []

        if len(receipt_files) == 0:
            st.info("لا توجد إيصالات محفوظة بعد.")
        else:
            # زرار تنزيل كل الإيصالات كملف واحد مضغوط
            zip_buffer = BytesIO()
            with zipfile.ZipFile(zip_buffer, "w") as zf:
                for fname in receipt_files:
                    zf.write(os.path.join(RECEIPTS_DIR, fname), arcname=fname)
            st.download_button(
                label=f"⬇️ تنزيل كل الإيصالات ({len(receipt_files)} صورة) كملف مضغوط",
                data=zip_buffer.getvalue(),
                file_name="receipts.zip",
                mime="application/zip"
            )

            with st.expander("عرض الإيصالات واحداً واحداً"):
                for fname in receipt_files:
                    st.image(os.path.join(RECEIPTS_DIR, fname), caption=fname, width=300)
