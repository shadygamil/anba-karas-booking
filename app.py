...
# ==================== إعدادات الرحلة والمهل الزمنية ====================
TRIP_DATE = ""            # مثال: "2026-10-20" — املاها لو عايز تحدد تاريخ الرحلة (اختياري)
TRIP_LOCATION = ""        # مثال: "دير الأنبا كاراس — وادي الريان" (اختياري)
TIMEZONE_NAME = "Africa/Cairo"   # توقيت مصر

# مهلة سداد باقي المبلغ: 15 يوم من تاريخ الحجز (زي ما هو مكتوب في الضوابط)
PAYMENT_DEADLINE_DAYS = 15
WARN_DAYS_BEFORE_DEADLINE = 7       # بداية التنبيه المبكر
URGENT_DAYS_BEFORE_DEADLINE = 3     # تنبيه عاجل (فاضل 3 أيام أو أقل)

AR_MONTHS = ["يناير", "فبراير", "مارس", "أبريل", "مايو", "يونيو", "يوليو", "أغسطس", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"]

# اللوجو لازم يكون موجود في نفس فولدر app.py باسم logo.png
LOGO_PATH = Path(__file__).parent / "logo.png"

def render_logo(width=170):
    """يعرض لوجو لجنة الرحلات، ولو الملف مش موجود مبيوقفش التطبيق"""
    try:
        if LOGO_PATH.exists():
...
def now_local():
    """الوقت الحالي بتوقيت مصر (القاهرة)"""
    try:
        return datetime.now(ZoneInfo(TIMEZONE_NAME))
    except Exception:
        return datetime.now()

def fmt_ar_datetime(dt):
    """يعلّم التاريخ والوقت بصيغة عربية مقروءة"""
    if dt is None:
        return "غير معروف"
    try:
        t = dt.strftime("%I:%M %p").replace("AM", "صباحاً").replace("PM", "مساءً")
    except Exception:
        t = ""
    return f"{dt.day} {AR_MONTHS[dt.month - 1]} {dt.year} - {t}"

def parse_booking_datetime(value):
    """يقرا تاريخ الحجز المسجل ويرجّعه كـ datetime، ولو مش قادر يرجّع None"""
    if value is None:
        return None
...
def get_payment_deadline(booking_date_value):
    """بيرجّع (هل لسه في المهلة، الأيام المتبقية، تاريخ غلق السداد) محسوبين من تاريخ الحجز"""
    booked = parse_booking_datetime(booking_date_value)
    if booked is None:
        return False, None, None
    deadline = (booked + timedelta(days=PAYMENT_DEADLINE_DAYS)).replace(hour=23, minute=59, second=59, microsecond=0)
    now = now_local().replace(tzinfo=None)
    seconds_left = (deadline - now).total_seconds()
    days_left = int(math.ceil(seconds_left / 86400.0))
    return seconds_left > 0, days_left, deadline
...
# إعدادات الصفحة
st.set_page_config(page_title="حجز مقاعد أسرة الأنبا كاراس", page_icon="🚌", layout="centered")

# ==================== شاشة شروط وضوابط الحجز والاسترداد (إلزامية) ====================
# النص ده بيتعرض أول حاجة، ومفيش أي حاجة من صفحة الحجز بتظهر غير بعد الموافقة عليه.
TERMS_TEXT_HTML = """
<div dir="rtl" style="direction:rtl; text-align:right; font-size:17px; line-height:2; background-color:#f8fafc; border:1px solid #dfe6ef; border-radius:12px; padding:18px 22px;">
<h2 style="text-align:right; margin-top:0;">📜 شروط وضوابط الحجز والاسترداد</h2>
<ol style="padding-right:22px; margin:0;">
<li style="margin-bottom:10px;"><b>جدية الحجز:</b> يُدفع 50% من قيمة الرحلة كمقدم (125 جنيهاً)، ولا يُعتبر المكان محجوزاً إلا بعد استكمال سداد إجمالي قيمة الرحلة البالغة 250 جنيهاً.</li>
<li style="margin-bottom:10px;"><b>استكمال باقي المبلغ:</b> يجب سداد باقي المبلغ في موعد أقصاه 15 يوماً قبل موعد الرحلة؛ وفي حال التخلف عن السداد، يُلغى الحجز تلقائياً ويُسقط حق المشترك في المقعد ومبلغ جدية الحجز.</li>
<li style="margin-bottom:10px;"><b>سياسة الإلغاء من قِبل المشترك:</b><ul style="padding-right:22px; margin:6px 0 0 0;"><li style="margin-bottom:6px;">إذا كان الإلغاء قبل الرحلة بأكثر من 15 يوماً: يُسترد مبلغ جدية الحجز كاملاً.</li><li>إذا كان الإلغاء خلال الـ 15 يوماً الأخيرة قبل الرحلة أو في حال عدم الحضور: لا يُسترد مقدم الحجز نهائياً.</li></ul></li>
<li><b>إلغاء الرحلة من قِبل اللجنة:</b> في حال إلغاء الرحلة لأي سبب إداري، يُرد كامل المبلغ للمشاركين فوراً وبدون أي خصم.</li>
</ol>
</div>
"""

def render_terms_screen():
    """تعرض شاشة الشروط والضوابط الإلزامية، وزر الانتقال مبيظهرش غير بعد تفعيل مربع الموافقة"""
    render_logo(width=180)
    st.title("🚌 حجز رحلة أسرة الأنبا كاراس")
    st.caption("لجنة الرحلات")
    st.warning("⚠️ برجاء قراءة شروط وضوابط الحجز والاسترداد بالكامل قبل الدخول إلى صفحة حجز المقاعد.")
    st.markdown(TERMS_TEXT_HTML, unsafe_allow_html=True)
    st.markdown("---")

    agreed = st.checkbox("أوافق على شروط وضوابط الحجز والاسترداد بالكامل، وأُقِرّ بأنني قرأتها وفهمتها.", key="terms_agree_cb")

    if agreed:
        if st.button("➡️ الانتقال إلى صفحة حجز المقاعد", type="primary"):
            st.session_state['terms_accepted'] = True
            st.rerun()
    else:
        st.info("لازم توافق على الشروط والضوابط الأول، وبعدها هيظهر زر الانتقال لصفحة الحجز.")

# حالة الموافقة على الشروط (افتراضياً: لم تتم الموافقة)
if 'terms_accepted' not in st.session_state:
    st.session_state['terms_accepted'] = False

# طالما المستخدم موافقش، بنعرض الشاشة دي بس ونوقف تنفيذ باقي الكود (صفحة الحجز مش بتتبني أصلاً)
...
# زر الرجوع لمراجعة الشروط والضوابط في أي وقت
col_back, _sp = st.columns([1, 3])
with col_back:
    back_clicked = st.button("🔙 رجوع للشروط", key="back_to_terms_btn", help="يرجّعك لشاشة شروط وضوابط الحجز والاسترداد")
if back_clicked:
    # نرجّع المستخدم لشاشة الشروط، ونصفّر الموافقة والمقعد المختار عشان يراجع من جديد
    st.session_state['terms_accepted'] = False
    st.session_state['selected_seat'] = None
    st.rerun()
st.caption("تقدر تراجع «شروط وضوابط الحجز والاسترداد» في أي وقت من زر الرجوع اللي فوق.")
st.markdown("---")

DB_FILE = "bookings.csv"
COLUMNS = ["الاسم رباعي", "رقم الموبايل", "رقم الكرسي", "المبلغ المدفوع", "تاريخ الحجز"]
RECEIPTS_DIR = "receipts"
os.makedirs(RECEIPTS_DIR, exist_ok=True)

# سجل الإقرارات (توثيق موافقة المستخدم على الشروط وسياسة الاسترداد مع الوقت)
CONSENTS_LOG_FILE = "consents_log.csv"
CONSENT_COLUMNS = ["الاسم رباعي", "رقم الموبايل", "رقم الكرسي", "تاريخ الحجز", "وقت الإقرار", "الشروط المقبول عليها"]

def log_consent(name, phone, seat, booking_date_str):
    """يسجّل إقرار المستخدم بالشروط وسياسة الاسترداد مع وقت الإقرار (ضمان قانوني عند أي نزاع)"""
    row = pd.DataFrame(
        [[name, phone, seat, booking_date_str, now_local().strftime("%Y-%m-%d %H:%M:%S"),
          "شروط وضوابط الحجز والاسترداد + إقرار سياسة الاسترداد"]],
        columns=CONSENT_COLUMNS,
    )
    if not os.path.exists(CONSENTS_LOG_FILE):
        row.to_csv(CONSENTS_LOG_FILE, index=False)
    else:
        row.to_csv(CONSENTS_LOG_FILE, mode="a", header=False, index=False)
...
# ترحيل تلقائي: عمود "تاريخ الحجز" (مهم لحساب مهلة سداد المتبقي 15 يوم)
if "تاريخ الحجز" not in df_bookings.columns:
    df_bookings["تاريخ الحجز"] = ""
    df_bookings.to_csv(DB_FILE, index=False)
df_bookings["تاريخ الحجز"] = df_bookings["تاريخ الحجز"].fillna("").astype(str)
...
st.caption("من تاريخ ووقت الحجز بنحسب آخر موعد لسداد باقي المبلغ (15 يوم).")
col_d, col_t = st.columns(2)
with col_d:
    trip_date = st.date_input("تاريخ الحجز", value=now_local().date(), key=f"trip_date_{st.session_state['form_counter']}")
with col_t:
    trip_time = st.time_input("وقت الحجز", value=now_local().time().replace(second=0, microsecond=0), key=f"trip_time_{st.session_state['form_counter']}")
...
# الصفوف العادية (من 1 إلى 44: كل صف 4 مقاعد: مقعدين، ممر في النص، مقعدين)
...
# الصف الأخير (من 45 إلى 49: 5 مقاعد جنب بعض تماماً مثل الصورة)
st.write("---") 
...
# 🔒 إقرار خاص بسياسة الاسترداد — لازم يتفعّل قبل ما زر تأكيد الحجز يشتغل
st.markdown("#### 🔒 إقرار سياسة الاسترداد")
st.markdown(
    "<div dir='rtl' style='direction:rtl; text-align:right; background-color:#fff8e1; border:1px solid #f0d9a0; border-radius:10px; padding:12px 16px; font-size:15px; line-height:1.9;'>"
    "بالتأكيد على حجزي، أنا مُقرّ بأنني قرأت وفهمت سياسة الاسترداد: "
    "<b>يُسترد مبلغ جدية الحجز كاملاً</b> إذا كان الإلغاء قبل الرحلة بأكثر من 15 يوماً، "
    "و<b>لا يُسترد مقدم الحجز نهائياً</b> إذا كان الإلغاء خلال الـ 15 يوماً الأخيرة قبل الرحلة أو في حال عدم الحضور، "
    "مع التزامي بسداد باقي المبلغ قبل موعد الرحلة بـ 15 يوماً على الأقل."
    "</div>",
    unsafe_allow_html=True,
)
refund_agreed = st.checkbox(
    "أُقِرّ بأنني قرأت سياسة الاسترداد وأوافق عليها وأتحمّل نتائجها.",
    key=f"refund_agreed_{st.session_state['form_counter']}",
)
if not refund_agreed:
    st.caption("⬆️ لازم تعلّم على مربع الإقرار ده الأول عشان زر «تأكيد الحجز» يشتغل.")

if st.button("تأكيد الحجز", disabled=not refund_agreed):
    if not refund_agreed:
        st.error("لازم توافق على «إقرار سياسة الاسترداد» الأول قبل تأكيد الحجز.")
        ...
        # نحسب تاريخ ووقت الحجز ونحفظه مع صف الحجز (عليه بتتحسب مهلة الـ 15 يوم)
        trip_dt = datetime.combine(trip_date, trip_time)
        booking_date_str = trip_dt.strftime("%Y-%m-%d %H:%M")
        # حفظ الحجز الجديد
        new_data = pd.DataFrame([[name, phone, selected_seat, amount_paid, booking_date_str]], columns=COLUMNS)
        ...
        # نسجّل إقرار المستخدم بالشروط وسياسة الاسترداد مع وقت الإقرار
        log_consent(name, phone, selected_seat, booking_date_str)
        remaining = TOTAL_PRICE - amount_paid
        if remaining > 0:
            deadline_dt = (trip_dt + timedelta(days=PAYMENT_DEADLINE_DAYS)).replace(hour=23, minute=59, second=59, microsecond=0)
            success_msg = (f"تم حجز المقعد رقم {selected_seat} بنجاح! باقي عليك {remaining} جنيه، تقدر تكملها من قسم 'استكمال دفع حجز' تحت. "
                           f"⏳ آخر موعد لسداد المتبقي: {fmt_ar_datetime(deadline_dt)}")
        else:
        ...
        st.session_state['selected_seat'] = None
        st.session_state['form_counter'] += 1  # يصفّر الاسم والرقم والإيصال والمربعات المرفوعة
        st.rerun()
...
if len(booked_seats) > 0:
    ...
    if complete_phone:
        ...
        if len(matches) == 0:
            ...
            booking_date_val = df_bookings.loc[row_idx[0], "تاريخ الحجز"] if "تاريخ الحجز" in df_bookings.columns else ""
            ...
            if remaining_now <= 0:
                ...
                # ⏳ تنبيه المستخدم قبل انتهاء مهلة الـ 15 يوم (من تاريخ الحجز)
                is_open, days_left, deadline_dt = get_payment_deadline(booking_date_val)
                if deadline_dt is not None and not is_open:
                    st.error(
                        f"⛔ انتهت مهلة سداد المتبقي (آخر ميعاد كان {fmt_ar_datetime(deadline_dt)}). "
                        f"المتبقي عليك {remaining_now} جنيه — برجاء التواصل مع اللجنة فوراً."
                    )
                elif deadline_dt is not None and days_left <= URGENT_DAYS_BEFORE_DEADLINE:
                    st.warning(
                        f"⏳ تنبيه عاجل: فاضل {days_left} يوم بس على آخر موعد لسداد المتبقي.\n\n"
                        f"• المتبقي عليك: **{remaining_now} جنيه**\n"
                        f"• آخر موعد لسداد المتبقي: **{fmt_ar_datetime(deadline_dt)}**\n\n"
                        "لو المبلغ ما اتسددش قبل الموعد ده، الحجز بيتلغى تلقائياً ويُسقط حق المشترك في المقعد ومبلغ جدية الحجز."
                    )
                elif deadline_dt is not None:
                    st.info(f"🗓️ آخر موعد لسداد المتبقي: **{fmt_ar_datetime(deadline_dt)}** — فاضل {days_left} يوم.")
                else:
                    st.caption("ℹ️ مهلة السداد مش محسوبة لأن تاريخ الحجز مش مسجل، برجاء التواصل مع اللجنة.")
...
with st.expander("🔐 لوحة الإدارة (للمسؤول فقط)"):
    ...
    if admin_pass_input == ADMIN_PASSWORD:
        ...
        if "تاريخ الحجز" not in df_all.columns:
            df_all["تاريخ الحجز"] = ""
        df_all["تاريخ الحجز"] = df_all["تاريخ الحجز"].fillna("").astype(str)

        # حالة مهلة سداد المتبقي لكل حجز (15 يوم من تاريخ الحجز)
        deadline_dates, deadline_status = [], []
        for _, row in df_all.iterrows():
            _is_open, _days_left, _deadline_dt = get_payment_deadline(row.get("تاريخ الحجز", ""))
            if row["المتبقي"] <= 0:
                deadline_dates.append("—")
                deadline_status.append("✅ مسدد بالكامل")
            elif _deadline_dt is None:
                deadline_dates.append("—")
                deadline_status.append("⚠️ تاريخ الحجز غير معروف")
            elif not _is_open:
                deadline_dates.append(fmt_ar_datetime(_deadline_dt))
                deadline_status.append(f"❌ عدّى مهلة السداد (متأخر {abs(_days_left)} يوم)")
            elif _days_left <= URGENT_DAYS_BEFORE_DEADLINE:
                deadline_dates.append(fmt_ar_datetime(_deadline_dt))
                deadline_status.append(f"🔴 عاجل: فاضل {_days_left} يوم")
            elif _days_left <= PAYMENT_DEADLINE_DAYS:
                deadline_dates.append(fmt_ar_datetime(_deadline_dt))
                deadline_status.append(f"🟡 فاضل {_days_left} يوم")
            else:
                deadline_dates.append(fmt_ar_datetime(_deadline_dt))
                deadline_status.append("🟢 في الموعد")
        df_all["موعد غلق السداد"] = deadline_dates
        df_all["حالة المهلة"] = deadline_status
        ...
        display_columns = [
            "رقم الكرسي", "الاسم رباعي", "رقم الموبايل", "تاريخ الحجز", "المبلغ المدفوع", "المتبقي",
            "موعد غلق السداد", "حالة المهلة", "عدد الإيصالات", "حالة آخر دفعة"
        ]
        st.dataframe(df_all[display_columns], use_container_width=True)

        # تنبيهات للإدارة بالحجوزات اللي قربت على مهلة السداد أو عدّتها
        overdue_rows = df_all[df_all["حالة المهلة"].astype(str).str.startswith("❌")]
        urgent_rows = df_all[df_all["حالة المهلة"].astype(str).str.contains("عاجل")]
        if len(overdue_rows) > 0:
            st.error(
                "⛔ في " + str(len(overdue_rows)) + " حجز عدّى مهلة السداد (15 يوم من تاريخ الحجز) — دول بيتم إلغاء حجزهم تلقائياً: "
                + " ، ".join(f"مقعد {r['رقم الكرسي']} ({r['الاسم رباعي']})" for _, r in overdue_rows.iterrows())
            )
        if len(urgent_rows) > 0:
            st.warning(
                "⏳ في " + str(len(urgent_rows)) + " حجز فاضلهم 3 أيام أو أقل على مهلة السداد، يُفضّل التواصل معاهم: "
                + " ، ".join(f"مقعد {r['رقم الكرسي']} ({r['رقم الموبايل']})" for _, r in urgent_rows.iterrows())
            )
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zf:
            ...
            if os.path.exists(CONSENTS_LOG_FILE):
                zf.write(CONSENTS_LOG_FILE, arcname="consents_log.csv")
        ...
        st.markdown("---")
        st.markdown("**🔒 سجل إقرارات الشروط وسياسة الاسترداد:**")
        if os.path.exists(CONSENTS_LOG_FILE):
            with open(CONSENTS_LOG_FILE, "rb") as cf:
                consents_bytes = cf.read()
            st.download_button(
                label="⬇️ تنزيل سجل الإقرارات (CSV)",
                data=consents_bytes,
                file_name="consents_log.csv",
                mime="text/csv",
                key="download_consents"
            )
        else:
            st.caption("لسه مفيش إقرارات مسجلة.")
