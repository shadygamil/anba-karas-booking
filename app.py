...
with st.expander("🔐 لوحة الإدارة (للمسؤول فقط)"):
    ...
    if admin_pass_input == ADMIN_PASSWORD:
        ...
        # نحسب عدد الدفعات وتاريخ آخر دفعة لكل حجز (ملخص سريع بس، التفاصيل الكاملة تحت)
        ...
        df_all["عدد الدفعات"] = payments_count
        ...
        st.markdown("---")
        st.subheader("🧾 سجل كل الدفعات لكل حجز (كل دفعة على حدة)")
        if len(df_all) > 0:
            row_labels = [
                f"مقعد {row['رقم الكرسي']} - {row['الاسم رباعي']} - {row['عدد الدفعات']} دفعة/دفعات"
                for _, row in df_all.iterrows()
            ]
            chosen_label = st.selectbox("اختر الحجز اللي عايز تشوف كل دفعاته بالتفصيل", options=row_labels, key="admin_choose_booking_history")
            ...
            if len(booking_logs) == 0:
                st.warning("مفيش سجل دفعات مفصّل لهذا الحجز (يمكن كان حجز قديم قبل إضافة السجل).")
            else:
                # نرقم كل دفعة (دفعة 1، دفعة 2، ...) بترتيب حصولها
                history_display = booking_logs[
                    ["المبلغ المدخل يدوياً", "المبلغ المكتشف من الإيصال", "حالة التطابق", "ملف الإيصال"]
                ].copy()
                history_display.insert(0, "رقم الدفعة", [f"دفعة {i+1}" for i in range(len(history_display))])
                st.dataframe(history_display, use_container_width=True)

                running_total = booking_logs["المبلغ المدخل يدوياً"].cumsum().iloc[-1]
                st.caption(f"إجمالي المدفوع من خلال هذه الدفعات: {running_total} جنيه (لازم يطابق عمود «المبلغ المدفوع» في الحجز: {chosen_row['المبلغ المدفوع']} جنيه)")

                st.markdown("**عرض صورة إيصال دفعة معيّنة من الدفعات دي:**")
                receipt_labels = [f"دفعة {i+1} - {row['حالة التطابق']}" for i, row in booking_logs.iterrows()]
                chosen_receipt_label = st.selectbox("اختر الدفعة", options=receipt_labels, key="admin_choose_specific_receipt")
                receipt_idx = receipt_labels.index(chosen_receipt_label)
                chosen_filename = booking_logs.iloc[receipt_idx]["ملف الإيصال"]
                ...
                if chosen_filename and os.path.exists(chosen_filepath):
                    ...
                    st.warning("مفيش صورة إيصال محفوظة لهذه الدفعة (أو اتمسحت مع إعادة تشغيل السيرفر).")
